...
import { X, ShoppingBag, AlertTriangle, Loader2, Trash2, Scale, Info } from "lucide-react";
...
export default function CartDrawer({ onClose, cartItems, setCartItems }: CartDrawerProps) {
  ...
  // Motor de cálculo: Total agora é apenas PRODUTO + FRETE
  const calculos = useMemo(() => {
    const freteYen = calculateShippingYen(estimatedWeight);
    const valorFreteBRL = freteYen * YEN_TO_BRL_RATE;
    
    // O total exibido para o cliente no carrinho
    const totalGeral = subtotalProdutos + valorFreteBRL;

    // Calculamos a taxa de redirecionamento por fora para enviar no WhatsApp ou banco, se necessário
    const subtotalYen = YEN_TO_BRL_RATE > 0 ? subtotalProdutos / YEN_TO_BRL_RATE : 0;
    let taxaServicoYen = 0;
    if (subtotalYen > 0) {
      taxaServicoYen = subtotalYen > 20000 ? subtotalYen * 0.20 : 4000;
    }
  ...
  const handleWhatsApp = async () => {
    ...
    // No WhatsApp enviamos o resumo limpo do carrinho e lembramos que a taxa é acertada depois
...
\n---
\nTOTAL DO CARRINHO: R$ ${calculos.totalGeral.toFixed(2)}
\n\n*Estou ciente da tabela de redirecionamento vista no carrinho e que o frete exato + taxa de serviço serão confirmados após o fechamento da caixa.*`;
    ...
    <div className="fixed inset-0 z-[200] flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-white h-full shadow-xl flex flex-col text-left">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {cartItems.length === 0 ? (
            ...
            <>
              ...
              {/* TABELA DE REDIRECIONAMENTO EXPLICATIVA */}
              <div className="bg-blue-50/60 border border-blue-100 p-3 rounded-xl space-y-2">
                <h3 className="font-black text-blue-900 text-[10px] uppercase flex items-center gap-1.5 tracking-wider">
                  <Info className="w-3.5 h-3.5 text-blue-600" /> Taxa de Redirecionamento
                </h3>
                <p className="text-[11px] text-blue-800 leading-tight">
                  Cobrada por caixa consolidada diretamente no fechamento do seu envio:
                </p>
                <div className="w-full bg-white rounded-lg border border-blue-100 text-[11px] overflow-hidden">
                  <div className="grid grid-cols-2 bg-blue-100/50 p-2 font-bold text-blue-900 border-b border-blue-100">
                    <span>Valor da Compra</span>
                    <span className="text-right">Taxa de Serviço</span>
                  </div>
                  <div className="grid grid-cols-2 p-2 text-gray-600 border-b border-slate-100">
                    <span>Até ¥20.000</span>
                    <span className="text-right font-semibold text-gray-900">¥4.000 <span className="text-gray-400 font-normal">(~R$ ${(4000 * YEN_TO_BRL_RATE).toFixed(0)})</span></span>
                  </div>
                  <div className="grid grid-cols-2 p-2 text-gray-600">
                    <span>Mais de ¥20.000</span>
                    <span className="text-right font-semibold text-gray-900">20% <span className="text-gray-400 font-normal">do valor</span></span>
                  </div>
                </div>
              </div>
            </>
        ...
        <div className="p-4 border-t bg-gray-50 space-y-3">
          {/* RESUMO DO CARRINHO APENAS PRODUTO + FRETE */}
